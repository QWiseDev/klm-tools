...
KLM Tools 是一个本地工具集合页面。当前已接入：

- 抖音去水印：粘贴 `https://v.douyin.com/...` 短链后，可以解析可用视频分辨率、在线播放预览并下载。
- Token 格式互转：Codex `auth.json`、CPA、Sub2API 三向互转，纯前端离线运行。
...
```text
http://127.0.0.1:8765/?tool=token-converter
```
...
https://your-domain.example/?tool=token-converter
```
...
web/
  ...
  token-converter.html           # Token 格式互转静态工具页
